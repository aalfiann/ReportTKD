                                    ...
                                    <form method="get" action="<?=$_SERVER['PHP_SELF']?>">
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label class="hidden-md hidden-lg">No Handphone</label>
                                                <input name="phone" type="text" class="form-control border-input text-uppercase" placeholder="No Handphone Anda" value="" required>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                        ...
                                        <div class="col-md-1" hidden>
                                            <div class="form-group">
                                                <input name="m" type="text" value="6" hidden>
                    ...
                    <?php if(isset($_GET['phone'])) 
{
    ...
    $url = $api."?mode=historinonmember&phone=".$_GET['phone']."&tanggalawal=".$_GET['tanggalawal']."&tanggalakhir=".$_GET['tanggalakhir']."&page=".$_GET['page']."&itemsperpage=".$_GET['itemsperpage']."";	
	...
	if (!empty($data))
    {
        if ($data->{'status'} == "success")
        {
            echo '<div class="col-md-12">
                        <div class="card card-plain">
                            <div class="header">
                                <h4 class="title text-uppercase">ID: '.$_GET['phone'].'</h4>
                            ...
                            <div class="row">
                                <div class="col-md-2 text-center" '.(($data->metadata->{'page_now'}<2)?"hidden":"").'>
                                    <div class="btn btn-wd">
                                            <a href="'.$_SERVER['PHP_SELF'].'?m=6&mode=historinonmember&phone='.$_GET['phone'].'&tanggalawal='.$_GET['tanggalawal'].'&tanggalakhir='.$_GET['tanggalakhir'].'&page='.($data->metadata->{'page_now'}-1).'&itemsperpage='.$_GET['itemsperpage'].'" ><i class="ti-arrow-left"></i> Sebelumnya</a>
                                    </div>
                                </div>';
                                if($data->metadata->{'page_total'} > 1)
                                {
                                    ...
                                    <div class="text-center">
                                        ...
                                        <input name="m" type="text" value="6" hidden>
                                        <input name="mode" type="text" value="historinonmember" hidden>
                                        <input name="phone" type="text" value="'.$_GET['phone'].'" hidden>
                                ...
                                echo '<div class="col-md-2 text-center" '.(($data->metadata->{'page_now'}>=$data->metadata->{'page_total'})?"hidden":"").'>
                                    <div class="btn btn-wd">
                                        <a href="'.$_SERVER['PHP_SELF'].'?m=6&mode=historinonmember&phone='.$_GET['phone'].'&tanggalawal='.$_GET['tanggalawal'].'&tanggalakhir='.$_GET['tanggalakhir'].'&page='.($data->metadata->{'page_now'}+1).'&itemsperpage='.$_GET['itemsperpage'].'" > Berikutnya <i class="ti-arrow-right"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
