                                    ...
                                    <form method="get" action="<?=$_SERVER['PHP_SELF']?>">
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label class="hidden-md hidden-lg">ID Member</label>
                                                <input name="idmember" type="text" class="form-control border-input text-uppercase" placeholder="ID Member Anda" value="" required>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                        ...
                                        <div class="col-md-1" hidden>
                                            <div class="form-group">
                                                <input name="m" type="text" value="5" hidden>
                    ...
                    <?php if(isset($_GET['idmember'])) 
{
    ...
    $url = $api."?mode=historimember&idmember=".$_GET['idmember']."&tanggalawal=".$_GET['tanggalawal']."&tanggalakhir=".$_GET['tanggalakhir']."&page=".$_GET['page']."&itemsperpage=".$_GET['itemsperpage']."";	
	...
	if (!empty($data))
    {
        if ($data->{'status'} == "success")
        {
            echo '<div class="col-md-12">
                        <div class="card card-plain">
                            <div class="header">
                                <h4 class="title text-uppercase">ID: '.$_GET['idmember'].'</h4>
                            ...
                            <div class="row">
                                <div class="col-md-2 text-center" '.(($data->metadata->{'page_now'}<2)?"hidden":"").'>
                                    <div class="btn btn-wd">
                                            <a href="'.$_SERVER['PHP_SELF'].'?m=5&mode=historimember&idmember='.$_GET['idmember'].'&tanggalawal='.$_GET['tanggalawal'].'&tanggalakhir='.$_GET['tanggalakhir'].'&page='.($data->metadata->{'page_now'}-1).'&itemsperpage='.$_GET['itemsperpage'].'" ><i class="ti-arrow-left"></i> Sebelumnya</a>
                                    </div>
                                </div>';
                                if($data->metadata->{'page_total'} > 1)
                                {
                                    ...
                                    <div class="text-center">
                                        ...
                                        <input name="m" type="text" value="5" hidden>
                                        <input name="mode" type="text" value="historimember" hidden>
                                        <input name="idmember" type="text" value="'.$_GET['idmember'].'" hidden>
                               ...
                               echo '<div class="col-md-2 text-center" '.(($data->metadata->{'page_now'}>=$data->metadata->{'page_total'})?"hidden":"").'>
                                    <div class="btn btn-wd">
                                        <a href="'.$_SERVER['PHP_SELF'].'?m=5&mode=historimember&idmember='.$_GET['idmember'].'&tanggalawal='.$_GET['tanggalawal'].'&tanggalakhir='.$_GET['tanggalakhir'].'&page='.($data->metadata->{'page_now'}+1).'&itemsperpage='.$_GET['itemsperpage'].'" > Berikutnya <i class="ti-arrow-right"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
