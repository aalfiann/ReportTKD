                            ...
                            <div class="header">
                                    ...
                                    <form method="get" action="<?=$_SERVER['PHP_SELF']?>">
                                        ...
                                        <div class="col-md-1" hidden>
                                            <div class="form-group">
                                                <input name="m" type="text" value="12" hidden>
                    ...
                    <?php if(isset($_GET['idusercorporate'])) 
{
    ...
    $url = $api."?mode=pousercorporate&idusercorporate=".$_GET['idusercorporate']."&tanggalawal=".$_GET['tanggalawal']."&tanggalakhir=".$_GET['tanggalakhir']."&page=".$_GET['page']."&itemsperpage=".$_GET['itemsperpage']."";	
	...
	if (!empty($data))
    {
        if ($data->{'status'} == "success")
        {
            ...
            foreach ($data->results as $name => $value) 
	        {
                echo '<tr>';
                echo '<td>' . $n++ .'</td>';
                echo '<td>' . $value->{'Tanggal Pickup'} .'</td>';
			    echo '<td>' . $value->{'No Pickup Order'} .'</td>';
    			echo '<td>' . $value->{'Corporate'} .'</td>';
            	echo '<td>' . $value->{'Divisi'} .'</td>';
        	    echo '<td>' . $value->{'PIC Order'} .'</td>';
    			echo '<td>' . $value->{'Kg'} .'</td>';
                echo '<td>' . $value->{'Koli'} .'</td>';
    			echo '<td>' . $value->{'Jenis'} .'</td>';
            	echo '<td>' . $value->{'Kurir'} .'</td>';
        	    echo '<td>' . $value->{'Keterangan'} .'</td>';
    			echo '<td>' . $value->{'Petugas'} .'</td>';
	    		echo '</tr>';              
            }
            echo '</tbody>
                    </table>
                            ...
                            <div class="row">
                                <div class="col-md-2 text-center" '.(($data->metadata->{'page_now'}<2)?"hidden":"").'>
                                    <div class="btn btn-wd">
                                            <a href="'.$_SERVER['PHP_SELF'].'?m=12&mode=pousercorporate&idusercorporate='.$_GET['idusercorporate'].'&tanggalawal='.$_GET['tanggalawal'].'&tanggalakhir='.$_GET['tanggalakhir'].'&page='.($data->metadata->{'page_now'}-1).'&itemsperpage='.$_GET['itemsperpage'].'" ><i class="ti-arrow-left"></i> Sebelumnya</a>
                                    </div>
                                </div>';
                                if($data->metadata->{'page_total'} > 1)
                                {
                                    ...
                                    <div class="text-center">
                                        ...
                                        <input name="m" type="text" value="12" hidden>
                                        <input name="mode" type="text" value="pousercorporate" hidden>
                                ...
                                echo '<div class="col-md-2 text-center" '.(($data->metadata->{'page_now'}>=$data->metadata->{'page_total'})?"hidden":"").'>
                                    <div class="btn btn-wd">
                                        <a href="'.$_SERVER['PHP_SELF'].'?m=12&mode=pousercorporate&idusercorporate='.$_GET['idusercorporate'].'&tanggalawal='.$_GET['tanggalawal'].'&tanggalakhir='.$_GET['tanggalakhir'].'&page='.($data->metadata->{'page_now'}+1).'&itemsperpage='.$_GET['itemsperpage'].'" > Berikutnya <i class="ti-arrow-right"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
